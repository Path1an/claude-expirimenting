  ...
  return (
    <SortableList
      ...
      renderRow={(p) => <>
        ...
        <td className="px-4 py-3">
          <Link href={`/admin/pages/${p.id}`} className="text-indigo-400 hover:text-indigo-300 text-sm transition-colors">Edit</Link>
        </td>
      </>}
      emptyCell={<>No pages yet. <Link href="/admin/pages/new" className="text-indigo-400 hover:text-indigo-300">Create one →</Link></>}
    />
  );
}
