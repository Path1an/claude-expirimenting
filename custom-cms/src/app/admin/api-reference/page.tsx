const METHOD_COLORS: Record<string, string> = {
  ...
  OPTIONS:'bg-zinc-700/50 text-zinc-400 border border-zinc-600',
};

const AUTH_COLORS: Record<string, string> = {
  Public:    'bg-zinc-800 text-zinc-400',
  Protected: 'bg-indigo-500/15 text-indigo-400 border border-indigo-500/30',
};
...
function MethodBadge({ method }: { method: string }) {
  return (
    <span className={`inline-block font-mono text-xs font-bold px-2 py-0.5 rounded ${METHOD_COLORS[method] ?? 'bg-zinc-700 text-zinc-300'}`}>
      {method}
    </span>
  );
}
  ...
  return (
    <div className="max-w-4xl">
      <div className="mb-6">
        ...
        <p className="text-zinc-500 text-sm mt-1">All available headless API endpoints</p>
      </div>

      {/* Auth overview */}
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 mb-6">
        <h2 className="text-sm font-semibold text-zinc-100 mb-3">Authentication</h2>
        <p className="text-sm text-zinc-400 mb-3">
          Protected endpoints require a Bearer token in the <code className="text-indigo-300 bg-zinc-800 px-1 py-0.5 rounded text-xs">Authorization</code> header.
          Create tokens in <strong className="text-zinc-200">Settings → API Tokens</strong>.
        </p>
        <pre className="bg-zinc-950 rounded-lg p-4 text-xs font-mono text-zinc-300 overflow-x-auto">{`curl https://your-cms.com/api/posts \\
  -H "Authorization: Bearer cms_your_token_here"`}</pre>
        <p className="text-xs text-zinc-600 mt-3">
          Public GET endpoints and OPTIONS requests work without a token.
          CORS allowed origins are configured in <strong className="text-zinc-500">Settings → Site Settings</strong>.
        </p>
      </div>
        ...
        {groups.map(group => (
          <div key={group.title} className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
            <div className="px-5 py-3 border-b border-zinc-800">
              <h2 className="text-sm font-semibold text-zinc-100">{group.title}</h2>
            </div>
            <div className="divide-y divide-zinc-800">
              {group.endpoints.map((ep, i) => (
                <div key={i} className="px-5 py-4">
                  ...
                  <p className="text-sm text-zinc-400 mt-1">{ep.description}</p>
                  {ep.requestBody && (
                    <details className="mt-2">
                      <summary className="text-xs text-zinc-500 cursor-pointer hover:text-zinc-300 transition-colors">
                        Request body
                      </summary>
                      <pre className="mt-2 bg-zinc-950 rounded-lg p-3 text-xs font-mono text-zinc-300 overflow-x-auto whitespace-pre-wrap">
                        {ep.requestBody}
                      </pre>
                    </details>
                  )}
                  {ep.responseNote && (
                    <p className="text-xs text-amber-400/80 mt-2">⚠ {ep.responseNote}</p>
                  )}
                </div>
              ))}
            </div>
      ...
      <div className="mt-6 bg-zinc-900 border border-zinc-800 rounded-xl p-5">
        <h2 className="text-sm font-semibold text-zinc-100 mb-3">Response format</h2>
        <p className="text-sm text-zinc-400 mb-3">All endpoints return JSON. List endpoints return an array directly. Error responses follow:</p>
        <pre className="bg-zinc-950 rounded-lg p-4 text-xs font-mono text-zinc-300">{`{ "error": "Unauthorized" }   // 401
{ "error": "Not Found" }       // 404
