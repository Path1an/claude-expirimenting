...
export default function SortableList<T extends SortableItem>({
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
      {saving && <p className="text-xs text-zinc-500 px-4 py-2 border-b border-zinc-800">Saving order…</p>}
      <table className="w-full text-sm">
        ...
        <tbody ref={tbodyRef}>
            ...
            return (
              <tr
                ...
                className={`border-b border-zinc-800 last:border-0 transition-colors
                  ${isDragging ? 'opacity-40 bg-zinc-800' : 'hover:bg-zinc-800/50'}`}
              >
                <td className="px-4 py-3">
                  <span
                    ...
                    className="text-zinc-600 hover:text-zinc-400 cursor-grab active:cursor-grabbing select-none block"
          ...
          {items.length === 0 && (
            <tr><td colSpan={colSpan + 1} className="px-4 py-12 text-center text-zinc-600">{emptyCell}</td></tr>
          )}
        </tbody>
      </table>
    </div>
