  ...
  return (
    <SortableList
      ...
      headers={<>
        ...
        <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 dark:text-zinc-400 uppercase tracking-wider">Author</th>
      ...
      renderRow={(p) => <>
        ...
        <td className="px-4 py-3 text-gray-400 dark:text-zinc-500">{p.author ?? '—'}</td>
        ...
        <td className="px-4 py-3">
          <Link href={`/admin/posts/${p.id}`} className="text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 text-sm transition-colors">Edit</Link>
        </td>
      </>}
      emptyCell={<>No posts yet. <Link href="/admin/posts/new" className="text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300">Create one →</Link></>}
    />
  );
}
