        ...
        {post.tags && (
          <div className="flex gap-2 mb-4 flex-wrap">
            {post.tags.split(',').map((t) => (
              <span key={t} className="px-2.5 py-0.5 bg-indigo-50 text-indigo-600 rounded-full text-xs font-medium">
                {t.trim()}
              </span>
            ))}
          </div>
        )}
        <h1 className="text-4xl font-bold tracking-tight text-gray-900 leading-tight">{post.title}</h1>
